...
def bayes_conf_intervals(qts, alpha = 0.1, hpd = False, variance = False):  
    if hpd: 
...
def conformal_conf_intervals(alpha, model_r, X_cal2, r_cal2, y_cal2, y_hat_cal2, X_val, r_val, y_val, y_hat_val):   
    # calculate q_yhat 
...
if __name__ == '__main__': 
    ...
    if args.method == 'bayes':  
        ...
        with tqdm(total = len(alphas)) as pbar:
            for alpha in alphas:
                lower_bound, upper_bound, results = conformal_conf_intervals(alpha, model_r, X_cal2, r_cal2, y_cal2, y_hat_cal2, X_val, r_val, y_val, y_hat_val)
